...
// Begin macros from libtomcrypt: tomcrypt_macros.h
...
// Begin code from libtomcrypt: md5.c
...
void shared_module_md5_compress(md5_state *md, const unsigned char *buf)
{
...
void shared_module_md5_done(md5_state *md, unsigned char *out)
{
    int i;
    ...
    if (md->curlen > 56) {
        ...
        shared_module_md5_compress(md, md->buf);
        md->curlen = 0;
    }
    ...
    /* store length */
    STORE32L(md->length, md->buf+56);
    STORE32L(0, md->buf+60);
    shared_module_md5_compress(md, md->buf);
...
void shared_module_md5_process(md5_state* md, const uint8_t* buf, uint32_t len) {
    ...
    while (len > 0) {
        if (md->curlen == 0 && len >= MD5_BLOCKSIZE) {
           shared_module_md5_compress(md, buf);
           md->length     += MD5_BLOCKSIZE * 8;
           buf            += MD5_BLOCKSIZE;
           len            -= MD5_BLOCKSIZE;
        } else {
           uint32_t rest = MD5_BLOCKSIZE - md->curlen;
           n = rest < len ? rest : len;
           memcpy(md->buf + md->curlen, buf, (size_t) n);
           md->curlen     += (uint32_t) n;
           buf            += n;
           len            -= n;
           if (md->curlen == MD5_BLOCKSIZE) {
              shared_module_md5_compress(md, md->buf);
              md->length += 8*MD5_BLOCKSIZE;
              md->curlen = 0;
           }
       }
    }
